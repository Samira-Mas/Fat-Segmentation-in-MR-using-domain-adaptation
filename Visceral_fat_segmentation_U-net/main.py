...
def main():
		...
		if args.mode is 'train':
			print('[i] Image directory:         ', args.dicom_folder)
			...
			myGene = trainGenerator(4,'data/train','images','labels',data_gen_args,save_to_dir ='data/train/aug_New')
			model_v = unet()
			...
			testGene = testGenerator("data/validation/images","data/validation/labels",137)
			...
			print('[i] Image directory:         ', args.dicom_folder.replace('/train/','/test/'))
			print('[i] Output directory:       ', args.target_folder.replace('/train/','/test/'))
			image_name_arr = glob.glob(join(".../CyclGAN_MR2CT/Results/Test_results_A2B", "*.png"))
